...
describe("Query functions structure", () => {
  describe("transactions queries", () => {
    it("必要な関数がエクスポートされている", async () => {
      ...
      expect(typeof transactionsModule.getTransactionsByDateRange).toBe("function");
      expect(typeof transactionsModule.getMonthlyTransactionSummary).toBe("function");
  ...
  describe("subscriptions queries", () => {
    it("必要な関数がエクスポートされている", async () => {
      ...
      expect(typeof subscriptionsModule.getActiveSubscriptions).toBe("function");
      expect(typeof subscriptionsModule.getSubscriptionsDueToday).toBe("function");
      expect(typeof subscriptionsModule.updateNextPaymentDate).toBe("function");
      expect(typeof subscriptionsModule.deactivateSubscription).toBe("function");
      expect(typeof subscriptionsModule.getMonthlySubscriptionTotal).toBe("function");
      expect(typeof subscriptionsModule.calculateNextPaymentDate).toBe("function");
    });
  });
  ...
  describe("categories queries", () => {
    ...
    it("reorderCategoriesはupdateDisplayOrderのエイリアスである", async () => {
      ...
      // エイリアス関数が正しく設定されている
      expect(categoriesModule.reorderCategories).toBe(categoriesModule.updateDisplayOrder);
    });
  });
