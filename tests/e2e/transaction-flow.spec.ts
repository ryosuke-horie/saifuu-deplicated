...
test.describe("基本ページロード", () => {
	test("ダッシュボードページが正常にロードされる", async ({ page }) => {
		...
		await expect(
			page.locator('[data-testid="summary-cards"], [data-testid="summary-cards-loading"], [data-testid="summary-cards-error"]').first()
	...
	test("取引フォームページが正常にロードされる", async ({ page }) => {
		...
		// ページタイトルが表示されることを確認
		await expect(page.getByRole("heading", { name: "Transaction Form Demo" })).toBeVisible();
		
		// フォームが表示されることを確認
		await expect(page.getByRole("heading", { name: "支出を登録" })).toBeVisible();
	});
